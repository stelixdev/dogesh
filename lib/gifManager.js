...
async function findMemoryChannel(client) {
  const channelId = (process.env.GIF_MEMORY_CHANNEL_ID || process.env.MEMORY_CHANNEL_ID || '1548308507810603028').trim();
  if (channelId) {
    try {
...
async function init(client) {
  try {
    ...
    if (!memoryChannel) {
      console.log('[gifManager] No persistent memory channel configured (set GIF_MEMORY_CHANNEL_ID in env). Using local cache only.');
      return;
    }
