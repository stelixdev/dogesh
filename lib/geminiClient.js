  ...
  for (const { key, originalIndex } of keysToTry) {
    ...
    try {
      ...
      if (isRateLimit) {
        ...
        console.warn(`[Gemini Key ${keyMask} Timed Out]: ${errMsg}. Bypassing secondary key to avoid latency accumulation.`);
        putKeyOnCooldown(key);
        // Break out immediately so we don't spend another 8.5s on Key 2; let fallback answer quickly
        break;
      } else {
