...
// Cache GoogleGenerativeAI client instances per key to reuse HTTP connections
const clientCache = new Map();
function getGenAI(key) {
  if (!clientCache.has(key)) {
    clientCache.set(key, new GoogleGenerativeAI(key));
  }
...
async function geminiChatCompletion(systemInstruction, messages) {
  ...
  for (const { key, originalIndex } of keysToTry) {
    ...
    try {
      ...
      tracker.lastExecution.modelUsed = 'gemini-3.5-flash-lite';
      ...
      if (isRateLimit) {
        ...
        console.warn(`[Gemini Key ${keyMask} Timed Out]: ${errMsg}. Skipping secondary key to avoid lag.`);
        // Do NOT put on 3-minute cooldown on timeout so Gemini remains available
        break;
      } else {
